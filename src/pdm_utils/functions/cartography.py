...
from sqlalchemy import MetaData, select, ForeignKey, join, select
...
if __name__ == "__main__":
    ...
    alchemist.username = "root"
    alchemist.password = "phage"
    
    alchemist.build_engine()

    alchemist.build_metadata()

    alchemist.build_session()
    
    map = get_map(alchemist.metadata, "cds")

    proxy = alchemist.session.query(map).filter(Cds.genome_id.in_(["Trixie"]))

    #proxy = alchemist.session.query(map)

    results = proxy.all()

    result = results[0]

    print((result.genome_id, result.name))
